...
class CoverFlowLayout : UICollectionViewFlowLayout {
    ...
    var delegate: CoverFlowDelegate?
    ...
    func changeLayoutAttribute(attribute:UICollectionViewLayoutAttributes)
    {
        ...
        //비율에 따라 스케일과 투명도 변경
        scale = ratio * (1 - self.itemScale) + 1.0
        alpha = ratio * (1 - self.itemAlpha) + self.itemAlpha

        print(alpha)
        
        delegate?.textViewScale(self, attribute: attribute)
